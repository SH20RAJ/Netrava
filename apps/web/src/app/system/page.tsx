...
export default function SystemDiagnosticsAndScalingPage() {
  ...
  async function runCalculation() {
    try {
      const res = await fetch("/api/v1/system/scalability-calculator", {
        method: "POST",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs font-mono">
          <div className="space-y-1 p-3 rounded bg-surface-raised border border-surface-border">
            ...
            <input
              ...
              className="w-full accent-police-gold"
            />
          </div>
          ...
          <div className="space-y-1 p-3 rounded bg-surface-raised border border-surface-border">
            ...
            <input
              ...
              className="w-full accent-telemetry-green"
            />
          </div>
        ...
        {calcResult && (
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="p-3 rounded-lg bg-surface-raised border border-surface-border">
                ...
                <div className="text-lg font-bold font-mono text-alert-red mt-0.5">
                  {calcResult.centralized_raw_video_bandwidth_gbps} Gbps
                </div>
              ...
              <div className="p-3 rounded-lg bg-surface-raised border border-police-gold/40">
                ...
                <div className="text-lg font-bold font-mono text-white mt-0.5">
                  {calcResult.netrava_edge_to_cloud_bandwidth_mbps} Mbps
                </div>
                <div className="text-[10px] text-telemetry-green font-bold">
                  {calcResult.bandwidth_reduction_pct}% Bandwidth Saved!
                </div>
              </div>
              ...
              <div className="p-3 rounded-lg bg-surface-raised border border-surface-border">
                ...
                <div className="text-lg font-bold font-mono text-cyber-cyan mt-0.5">
                  {calcResult.total_statewide_inferences_per_sec.toLocaleString()} inf/s
                </div>
                <div className="text-[10px] text-slate-400 font-mono">
                  {calcResult.cameras_per_regional_hub} cams / district
                </div>
              </div>
              ...
              <div className="p-3 rounded-lg bg-surface-raised border border-surface-border">
                ...
                <div className="text-lg font-bold font-mono text-telemetry-green mt-0.5">
                  {calcResult.gpus_required_per_hub}x GPUs
                </div>
                <div className="text-[10px] text-slate-400 font-mono">
            ...
            <div className="p-4 rounded-lg bg-black/40 border border-surface-border text-xs space-y-2">
              ...
              <p className="text-slate-300 leading-relaxed">{calcResult.gpu_hardware_recommendation}</p>
              <p className="text-slate-300 leading-relaxed">{calcResult.storage_architecture_recommendation}</p>
            </div>
          </div>
        )}
      </div>
