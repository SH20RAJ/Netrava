...
export function CamerasRegistryView() {
  const [districtFilter, setDistrictFilter] = useState("ALL");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [search, setSearch] = useState("");
  const [selectedHealth, setSelectedHealth] = useState<any>(null);

  // SWR-driven camera registry fetch
  let apiUrl = "/api/v1/cameras?limit=100";
  if (districtFilter !== "ALL") apiUrl += `&district=${districtFilter}`;
  if (statusFilter !== "ALL") apiUrl += `&status=${statusFilter}`;

  const { data: cameras = [], isLoading, mutate } = useSWR<any[]>(
    apiUrl,
    fetcher,
  ...
  const handleInspectHealth = async (camId: string) => {
    try {
      const data = await fetcher(`/api/v1/cameras/${camId}/health`);
